...
% TODO fix reference
For each task (or subtask, as it is part of a rule task) defined in the rule context, \arara\ will interpret it and return the corresponding system command. The return types can be found in Section~\ref{foo}, on page~\pageref{foo}. In this specific scenario, there is just one task associated with the \rbox{pdftex} rule. Both task name and system command are informed:
