...
% The last clause in the next sentence doesn't sound quite right.
% Suggestion:
%   resulting in an expected \rbox{hello.pdf} file created using the same system call that tools like \rbox{latexmk} and \rbox{rubber} use.
Hurrah, we finally got our document properly compiled with a \TeX\ engine by the inner workings of our beloved tool, resulting in an expected \rbox{hello.pdf} file in the same fashion tools like \rbox{latexmk} and \rbox{rubber} generate. Observe that \arara\ works practically on other side of the spectrum: you need to tell it how and when to do a task.
