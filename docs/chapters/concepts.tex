...
% (Me being slightly pedantic.) With the document reshuffle, this first sentence isn't quite true as the reader has had a little contact in the introduction.
% Perhaps "Time for our first proper contact with \arara!" or perhaps just delete.
...
% The world ordering seems a little off in the penultimate sentence above.
% Suggestions:
%  [Minor single-sentence edit] A \emph{task} is an instantiated rule (through a directive).
%  [Full paragraph edit] This key holds the name of the \emph{task} (a rule instantiated through a directive) as a plain string. When running \arara, this value will be displayed in the output enclosed in parentheses.
...
\item[\varbox{reference}] This variable is introduced in version 4.0 of \arara\ and holds the canonical, absolute path representation of the \varbox{file} variable as a \rbox{File} object. This is useful if it's necessary to understand the hierarchical structure of a project. Since the reference is a Java object, we can use all methods available in the \rbox{File} class.
\end{description}
