...
% Sorry, I'm going to be a bit pedantic here.
% Above, you've defined the allowed set of boolean values: yes, true, 1, on and no, false, 0, off.
% Below, for isTrue, you have "is contained in the set of natural boolean values"
% Strictly speaking, this implies that isTrue returns true if the argument is any of the allowed boolean values listed above, which includes the false sub-set, and so gives the impression that isTrue("false") returns true.
% I recommend that for the isTrue methods, replace
%   is contained in the set of natural boolean values
% with
%   is contained in the sub-set of natural true boolean values
% and for the isFalse methods, replace
%   is not contained in the set of natural boolean values
% with
%   is contained in the sub-set of natural false boolean values
