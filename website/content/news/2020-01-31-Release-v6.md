...
it containing other directives. That mean, if you have the following in your
configuration:
...
> Quick tip: Remeber you can also use `getSession().get('environment:PATH')` and
> similar to get the `PATH` environment variable.
...
which in fact does not specify how to compile `file.tex` but it specifies three
things:
